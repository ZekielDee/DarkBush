...
void GCD_0(Circom_CalcWit* ctx,FrElement* lvar,FrElement* destination,int destination_size);
void isPrime_1(Circom_CalcWit* ctx,FrElement* lvar,FrElement* destination,int destination_size);
...
uint get_total_signal_no() {return 1991;}

uint get_number_of_components() {return 10;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 1519;}
...
Fr_neq(&expaux[0],&lvar[2],&lvar[3]); // line circom 12
while(Fr_isTrue(&expaux[0])){
Fr_gt(&expaux[0],&lvar[2],&lvar[3]); // line circom 13
if(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &lvar[2];
// load src
Fr_sub(&expaux[0],&lvar[2],&lvar[3]); // line circom 14
// end load src
...
PFrElement aux_dest = &lvar[3];
// load src
Fr_sub(&expaux[0],&lvar[3],&lvar[2]); // line circom 16
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
}
...
FrElement* circuitConstants = ctx->circuitConstants;
FrElement expaux[4];
Fr_eq(&expaux[0],&lvar[0],&circuitConstants[2]); // line circom 24
if(Fr_isTrue(&expaux[0])){
// return bucket
...
Fr_lt(&expaux[0],&lvar[1],&lvar[0]); // line circom 27
while(Fr_isTrue(&expaux[0])){
Fr_mod(&expaux[1],&lvar[0],&lvar[1]); // line circom 28
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "comp64";
mySubcomponents[aux_create+i] = aux_cmp_num;
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "comp32";
mySubcomponents[aux_create+i] = aux_cmp_num;
LessThan_1_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 69 ;
aux_cmp_num += 2;
...
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 3],&signalValues[mySignalStart + 3]); // line circom 52
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 4],&signalValues[mySignalStart + 4]); // line circom 53
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_add(&expaux[0],&signalValues[mySignalStart + 5],&signalValues[mySignalStart + 6]); // line circom 55
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
...
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 2],&signalValues[mySignalStart + 2]); // line circom 62
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1];
// load src
Fr_add(&expaux[0],&signalValues[mySignalStart + 5],&signalValues[mySignalStart + 6]); // line circom 63
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
// no need to run sub component
...
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[1]].signalStart + 0],&circuitConstants[1]); // line circom 65
...
// copying argument 0
Fr_copy(&lvarcall[0],&signalValues[mySignalStart + 9]);
// end copying argument 0
// copying argument 1
Fr_copy(&lvarcall[1],&signalValues[mySignalStart + 10]);
// end copying argument 1
GCD_0(ctx,lvarcall,&lvar[0],1);
// end call bucket
}
...
// copying argument 0
Fr_copy(&lvarcall[0],&signalValues[mySignalStart + 11]);
// end copying argument 0
isPrime_1(ctx,lvarcall,&signalValues[mySignalStart + 12],1);
// end call bucket
}

Fr_eq(&expaux[0],&signalValues[mySignalStart + 12],&circuitConstants[1]); // line circom 79
...
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[2]].signalStart + 0],&circuitConstants[2]); // line circom 84
if (!Fr_isTrue(&expaux[0])) std::cout << "Failed assert in template " << myTemplateName << " line 84. " <<  "Followed trace: " << ctx->getTrace(myId) << std::endl;
assert(Fr_isTrue(&expaux[0]));
{
